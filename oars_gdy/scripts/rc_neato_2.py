    ...
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            ...
            for event in gamepad.read_loop():
                ...
                if not event.type == 0:
                    if event.code == 1:
                        x_vel = int(translate(event.value, sailDownMax, sailUpMax, sailIn, sailOut))
                        self.cmd_vel.linear.x = x_vel
                    elif event.code == 2:
                        y_vel = int(translate(event.value, rudderLeftMax, rudderRightMax, rudderLeft, rudderRight))
